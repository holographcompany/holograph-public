// /src/app/holographs/[id]/utilties/page.tsx
"use client";
...
import UtilityModal from "@/app/_components/utilities/UtilityModal";
...
export default function UtilityPage() {
  ...
  const { isAuthorized, accessDenied, holographTitle, sectionName, isLoading: isAccessLoading } = useSectionAccess("utilities");

  const [accounts, setAccounts] = useState<Utility[]>([]);
  const [signedUrls, setSignedUrls] = useState<{ [key: string]: string }>({});
  const [selectedAccount, setSelectedAccount] = useState<Utility | null>(null);
  ...
  useEffect(() => {
    async function fetchAccounts() {
      try {
        const response = await apiFetch(`/api/utilities?holographId=${holographId}`);
  ...
  useEffect(() => {
    const fetchUrls = async () => {
      ...
      for (const acc of accounts) {
        if (acc.filePath) {
          try {
            const res = await apiFetch(`/api/generate-signed-url?filePath=${encodeURIComponent(acc.filePath)}&holographId=${encodeURIComponent(holographId)}&section=utilities`);
  ...
  const openModal = (account: Utility | null) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      await apiFetch(`/api/utilities/${id}`, {
        method: "DELETE",
  ...
  const refresh = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`/api/utilities?holographId=${holographId}`, { withCredentials: true });
    ...
    <div className="flex gap-6 p-8 max-w-6xl mx-auto">
      ...
      <div className="w-1/3 bg-white shadow-lg p-6 rounded-lg">
        <div className="flex flex-col gap-4">
          {isPrincipal && <button className="btn-primary" onClick={() => openModal(null)}>+ Add Utility</button>}
        ...
        <div className="mt-6 text-gray-700 text-sm space-y-2">
          {isPrincipal ? (
            <>
              <p>Use this section to list utilities such as electricity, water, gas, internet and others for each property you own or rent.</p>
              <p>You may also upload a document for each utility with additional information. </p>
            </>
          ) : (
            <p className="italic">
              You can view Utilities shared with you.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="w-2/3 bg-white shadow-lg p-6 rounded-lg">
        ...
        {isLoading ? (
          ...
          <p className="text-gray-500">No utilities added yet.</p>
        ) : (
          <table className="w-full mt-4 border-collapse border border-gray-300">
            <thead>
              <tr className="bg-gray-100 text-left">
                <th className="p-3 border border-gray-300">Utility Name</th>
      ...
      {selectedNote !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`bg-white p-6 rounded-lg shadow-lg ${isExpanded ? 'max-w-5xl w-full h-[90vh]' : 'max-w-md w-full max-h-[80vh]'} overflow-y-auto relative transition-all duration-300`}>
            <h2 className="text-lg font-semibold mb-4">Utility Notes</h2>
