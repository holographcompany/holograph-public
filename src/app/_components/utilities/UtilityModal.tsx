// /src/app/_components/utilities/UtilityModal.tsx
...
export default function UtilityModal({
  ...
  // allows the user to delete a file attached to a Utility record without deleting the whole record.
  const handleFileDelete = async () => {
    ...
    try {
      ...
      await apiFetch(`/api/utilities/${account.id}?fileOnly=true`, {
        method: "DELETE",
        headers: {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.name.trim()) {
      validationErrors.name = "Utility name is required.";
    }
    ...
    debugLog("🟢 Sending Utility FormData:", Object.fromEntries(formDataToSend.entries()));

    try {
      const csrfRes = await apiFetch("/api/csrf-token");
      const { csrfToken } = await csrfRes.json();
      await apiFetch(`/api/utilities`, {
        method: "POST",
        headers: {
          "x-csrf-token": csrfToken,
        },
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">
          {account ? "Edit Utility" : "Add New Utility"}
        </h2>

        {/* Account Name */}
        <label className="block text-gray-700 font-medium">Utility Name *</label>
        <input
          type="text"
