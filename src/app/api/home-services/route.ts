...
import { homeServiceSchema } from "@/validators/homeServiceSchema";
...
export const GET = withCors(async (req) => {
  ...
  try {
    ...
    if (!isPrincipal && !isDelegate) {
      return NextResponse.json({ error: "Forbidden — no access to these Home Services" }, { status: 403 });
    }

    const accounts = await prisma.homeService.findMany({
      where: { holographId },
    ...
    debugLog("✅ Decrypted Home Services:", decryptedAccounts);
...
export const POST = withCors(async (req) => {
  ...
  try {
    ...
    const homeServiceId = formData.get("id") as string | null; 
    const isNewDocument = !homeServiceId && !existingFilePath;
    createdBy = userId;
    updatedBy = userId;

    // ✅ Zod Validation
    try {
      homeServiceSchema.parse({ name, notes });
    } catch (err) {
    ...
    if (homeServiceId) {
      existingAccount = await prisma.homeService.findUnique({
        where: { id: homeServiceId },
      });
    } else if (!isNewDocument) {
      existingAccount = await prisma.homeService.findFirst({
        where: { holographId, filePath: existingFilePath || null },
      });
    }

    if (!isNewDocument && !existingAccount) {
      debugLog("⚠️ No Home Service found, preventing accidental duplication.");
      return NextResponse.json({ error: "Home Service record not found for update." }, { status: 404 });
    }
    ...
    if (isNewDocument) {
      debugLog("🆕 Creating Home Service...");
      const created = await prisma.homeService.create({
        data: {
          holographId,
          uploadedBy,
          createdBy,
      ...
      debugLog("✏️ Updating Home Service...");
      const updated = await prisma.homeService.update({
        where: { id: homeServiceId || existingAccount?.id },
        data: {
          uploadedBy,
          updatedBy,
          filePath: relativeFilePath || null,
    ...
    console.error("❌ Full error during Home Service create/update:", error);
