// /src/app/api/home-services/[id]/route.ts 
...
import { homeServiceSchema } from "@/validators/homeServiceSchema"; // ✅ Import schema
...
export const PUT = withCors(async (req, context) => {
  ...
  try {
    ...
    if (file) {
      ...
      const gcsPath = `home-services/${holographId}/${timestampedFileName}`;
    ...
    const updatedAccount = await prisma.homeService.update({
    ...
    debugLog("✅ Home Service updated:", updatedAccount.id);
...
export const DELETE = withCors(async (req, context) => {
  ...
  try {
    ...
    const record = await prisma.homeService.findUnique({
      where: { id: context.params.id },      select: { filePath: true, holographId: true },
    });

    if (!record) {
      return NextResponse.json({ error: "Home Service not found" }, { status: 404 });
    }

    // Ensure the user has permissions to modify this home service
    ...
    if (fileOnly) {
      ...
      debugLog(`🗑️ Database updated: filePath=null, uploadedBy=null for Home Service ${context.params.id}`);
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.homeService.delete({
        where: { id: context.params.id },      });

      debugLog(`🗑️ Deleted Home Service ${context.params.id} from database`);
    });
    return NextResponse.json({ success: true, message: "Home Service deleted" });
